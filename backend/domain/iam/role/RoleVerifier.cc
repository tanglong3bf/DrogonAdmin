...
Task<> RoleVerifier::verifyRoleNameNotDuplicated(string_view name) const
{
...
Task<> RoleVerifier::verifyRoleCodeNotDuplicated(string_view code) const
{
    ...
    const vector<int32_t> &newRoleIds) const
{
                            ...
                            }) | ranges_utils::to<vector>())
    {
        ...
        const size_t userCountInRole = userCount.contains(*role.roleId())
                                           ? userCount.at(*role.roleId())
                                           : 0;
                                ...
                                const Role & /* ignore */) const
{
    ...
    if (role.quotaType() == QuotaType::TotalLimit)
    {
        // 检查所有使用当前角色的用户数量是否超过限制
        const auto count =
            co_await userRepository_->countByRole(*role.roleId());
