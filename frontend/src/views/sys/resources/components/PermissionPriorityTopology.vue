...
const dots = computed(() => {
  ...
  // 按 action_id 固定排序，DOM 顺序永远不变，仅通过坐标控制视觉位置
...
const getSVGPoint = (e: MouseEvent): { x: number; y: number } => {
  ...
  pt.x = e.clientX
  pt.y = e.clientY
  // 矩阵逆变换：屏幕坐标 → SVG 内部坐标，不受边框、缩放、viewBox 影响
...
const handleMouseUp = (e: MouseEvent) => {
  ...
  if (targetDot && targetDot.id !== startDotId.value) {
    const high = startDotId.value!
    const low = targetDot.id
